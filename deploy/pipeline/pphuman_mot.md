...
## 视频预测服务
1. 使用`rtsp-simple-server`建立RTSP视频传输地址，可以用docker部署。
```docker
...
```
3. 运行检测脚本，根据自己的conda地址和环境名修改`run_pull_rtsp.sh`。
```bash
bash run_pull_rtsp.sh
```
4. 使用`push_rtsp.py`去推流测试，需要提前安装好`ffmpeg`
```python
python push_rtsp.py
```

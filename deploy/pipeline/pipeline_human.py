...
def count_rate(image, width, height):
    pixel_num = np.where(image == [0, 0, 0])[0].shape[0]
    black_rate = pixel_num/(width*height)
    return black_rate
...
class PipePredictor(object):
    ...
    def predict_video(self, image_list, width, height, fps, thread_idx=0):
        # mot
        # mot -> attr
        # mot -> pose -> action
        ...
        for i in range(0, frame_count):
            ...
            frame = image_list[i]
            """
            if count_rate(frame, width, height) > 0.6:
                frame_id += 1
                continue
            """
        ...
        # print(len(self.pipeline_res.get('mot')['boxes'][:, 0]))
        return len(self.collector.get_res().keys()),visualize_list
